...
- Progress record:
  ...
  - latest D281 blocker: workbook row `281` is source-backed to `wl -i wlX escanresults` `noise: ` parsing, so a temporary workbook-style first-WPA-target compare was trialed, but two isolated reruns (`20260410T155529962490` / `20260410T155932807150`) still failed to produce a stable same-target replay. The first replay matched 5G (`LlapiNoise5g=-100`, `WlNoise5g=-100`) but emitted no `WlNoise6g` and drifted on 2.4G (`LlapiNoise24g=-80`, `WlNoise24g=-77`); the second replay lost `WlNoise` on both 5G and 6G and still drifted on 2.4G (`-80` vs `-79`). So D281 remains blocked, the trial rewrite was rejected, and the committed YAML stays unchanged (`source.row=283` still stale) until a deterministic same-target replay exists
  - details recorded in `plugins/wifi_llapi/reports/D281_block.md`
  - latest D282 blocker: workbook row `282` also failed to freeze into a committed rewrite. Trial isolated rerun `20260410T163026194231` produced `5G a,n,ac,ax`, `6G ax,be`, and `2.4G b,g,n,ax`, but still no deterministic all-band same-target replay: 5G no longer matched the earlier EHT-shaped survey target, 6G emitted no same-target `WlOperatingStandards6g`, and 2.4G drifted to `LLAPI b,g,n,ax` vs `wl b,g,n,ax,be`. Controlled baseline probes also stayed inconsistent (`testpilot5G` LLAPI `a,n,ac,ax,be` vs wl `a,n,ac,ax`; `testpilot6G` absent in the replay snapshot; `testpilot2G` absent in LLAPI while wl still exposed an EHT-capable baseline BSSID). So D282 remains blocked, the trial rewrite was rejected, and the committed YAML stays unchanged (`source.row=284` still stale)
