...
| Offline summary reproject | `xlsx` + `md` + `html` + `json` | `wifi-llapi reproject-summary` reads an existing JSON artifact, validates the checked-in template workbook, fills `Wifi_LLAPI` raw results plus hidden projected summary buckets, and leaves the styled `Summary` sheet to calculate from template formulas in a new report folder without rerunning DUT/STA tests or modifying the source run folder |
...
- latest 0506 workbook audit continuation: active RID `74ada64b-2026-05-07T134956Z` has `D004 kickStation()` applied and validated (`Pass / Pass / Pass`, run `20260508T212440407797`), while `D006`, `D009`, `D012`, `D013`, `D014`, `D015`, `D016`, and `D017` are blocked as workbook/source/live authority conflicts. `D016 DownlinkBandwidth` and `D017 DownlinkMCS` both reached AP1 only after audit-gated exploratory removal of redundant `wpa_cli` gates; `iw dev wl0 link` plus `wl0 assoclist` proved driver association, but `WiFi.AccessPoint.1.AssociatedDevice.1.MACAddress?` returned object not found before the target getter could be read. AP5 showed the same wildcard `No data found` despite `wl2 assoclist`, AP3 exposed `AssociatedDevice.1`, and both exploratory gate edits were reverted through audit `verify-edit` / `apply`. Current audit buckets are `confirmed=151`, `applied=1`, `pending=145`, `block=118`, `needs_pass3=0`; next strict single-case Pass3 target is `D018`.
...
| 離線 summary 重新投影 | `xlsx` + `md` + `html` + `json` | `wifi-llapi reproject-summary` 讀取既有 JSON artifact、驗證 repo 內已提交的 template workbook，填入 `Wifi_LLAPI` raw result 與隱藏的 summary bucket，讓已套樣式的 `Summary` sheet 透過 template 公式自行計算，並在新資料夾寫出報告，不重跑 DUT/STA 測試也不修改來源 run 資料夾 |
