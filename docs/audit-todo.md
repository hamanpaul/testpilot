...
- `D483 Radio Stats WmmBytesSent AC_BK` is now aligned via official rerun `20260415T095013796898`
- object/api were already aligned to workbook `WiFi.Radio.{i}.Stats.WmmBytesSent.` / `AC_BK`, but the old source row `350` plus `getRadioStats() | grep AC_BK_Stats` replay is now retired in favor of workbook row `483` with direct `WiFi.Radio.{i}.Stats.WmmBytesSent.AC_BK?` getters
- the rerun exact-closes workbook `Pass / Pass / Pass` while keeping `diagnostic_status=Pass`: 5G/6G/2.4G direct getter values all match `wl0/wl1/wl2 wme_counters` `AC_BK` tx bytes at `0 / 0 / 0`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D483 Radio Stats WmmBytesSent AC_BK`
- targeted D483/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`); the D481 and D482 exploratory rewrites also cleared targeted runtime + command-budget guardrails before the official reruns exposed their 6G drifts. Compare therefore now moves to `368 / 420 full matches` / `52 mismatches` while metadata drifts stay at `43`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D484 Radio Stats WmmBytesSent AC_VI`
