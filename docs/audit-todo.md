...
- `D432 AccessPoint.Neighbour.{i}.OperatingClass` is now aligned via official rerun `20260415T044008021410`
- workbook authority is row `432`; the old skip placeholder is now replaced by a workbook-faithful AP-only neighbour lifecycle on `WiFi.AccessPoint.{i}.Neighbour.{i}.`, and the rerun exact-closes `ABSENT -> 0 -> ABSENT` on AP1/AP3/AP5 while the created entry reads back `OperatingClass=0` together with `11:22:33:44:55:66 / 36`、`11:22:33:44:55:77 / 1`、`11:22:33:44:55:88 / 11`; final status is `diagnostic_status=Pass`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed aligned case is now `D432 AccessPoint.Neighbour.{i}.OperatingClass`
- targeted D432/runtime + neighbour/skip-bucket guardrails passed; full repo regression is now `1662 passed`; compare is now `355 / 420 full matches` / `65 mismatches` / `50 metadata drifts`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D433 AccessPoint.Neighbour.{i}.PhyType`
