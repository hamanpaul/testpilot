...
- latest direct-stats continuation after that: `D324 BytesSent` remains blocked. The earlier exact-close rerun `20260411T010328768651` is no longer durable: detached full run `20260411T074146043202` already drifted on 6G, isolated rerun `20260411T190338070996` re-proved `direct == getSSIDStats()` while invalidating base `wlX if_counters txbyte`, and the follow-up official WDS-sum rerun `20260412T005627796136` still did not exact-close after switching the driver oracle to `wlX txbyte + Σwds* txbyte`. That rerun reduced the old large multi-band gaps, but the live shape became a sequential staircase — 5G `148970905 / 148971033 / 148971377`, then `149022147 / 149022215 / 149022559`; 2.4G also showed `157367729 / 157391543 / 157415359` on attempt 1 — and active 0403 pWHM source now proves why: direct `Stats.*` and `getSSIDStats()` both call `s_updateSsidStatsValues()` independently before serializing output. So the local rewrite stays rolled back and the next ready overall revisit returns to `D282`
...
- latest direct-stats continuation after that: `D333 PacketsSent` remains formally blocked in `plugins/wifi_llapi/reports/D333_block.md`. The stale workbook replay `20260411T194816992700` re-proved the loose `getSSIDStats()` overmatch and stale `/proc/net/dev_extstats` `$11` path; the source-backed trial `20260411T195140855058` fixed the extractor but still kept a fixed 5G `driver = direct + 5` drift, and the superseding official rerun `20260411T235643720137` re-proved that same 5G `+5` shape in the full runner path. A follow-up settle retrial `20260412T004816450100` materially narrowed the residual mismatch — attempt 1 failed only on 5G by `+1` (`324804 / 324804 / 324805`), while attempt 2 exact-closed 5G/2.4G and failed only on 6G by `+2` (`212232 / 212232 / 212234`) — but the official acceptance path still did not go green, so the local rewrite stays rolled back
...
- latest scan-results continuation after that: `D282 getScanResults() OperatingStandards` still remains blocked, but the blocker is now more concrete. Active 0403 source tracing in `pwhm-v7.6.38/src/RadMgt/wld_rad_scan.c` now shows `getScanResults()` serializes only `ssid->operatingStandards`, while sibling ubus method `WiFi.NeighboringWiFiDiagnostic()` serializes both `OperatingStandards` and `SupportedStandards` from the same `wld_scanResultSSID_t`. Live probes proved the sibling path is real — 2.4G first target `04:70:56:D2:22:4F` exact-closed as `b,g,n,ax` on both `getScanResults()` and `NeighboringWiFiDiagnostic().Result[]` — but the same diagnostic currently returns `FailedRadios = "wl0,wl1"` / `ReportingRadios = "wl2"`, so 5G/6G still have no durable same-target sibling replay. Therefore the committed YAML stays unchanged at stale row `284`, `plugins/wifi_llapi/reports/D282_block.md` is the superseding blocker authority, and the next ready runtime-triage case moves to `D324`
