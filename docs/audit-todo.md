...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=163`, `applied=8`, `pending=105`, `block=139`, `needs_pass3=0`
- latest blocker is `D058 UNIIBandsCapabilities`: workbook row 58 expects `Pass / Pass / Pass`, and source declares `AssociatedDevice.UNIIBandsCapabilities` as a read-only string with U-NII enumerations behind the AssociatedDevice read path; focused run `20260509T182432280364` did not reach the getter because case-local WPA3/SAE `sta_env_setup[48]` failed at `iw dev wl0 link` with `Not connected.` after retry/remediation; fixing stale setup plus tri-band UNIIBands coverage is outside the audit allowlist
