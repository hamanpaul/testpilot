...
- latest scan-results continuation after that: `D282 getScanResults() OperatingStandards` also remains blocked, and its blocker model is now narrower again. The active public ubus path is no longer treated as the old Broadcom `_wldm_get_standards()` helper: current 0403 source tracing shows the nl80211 parser fills both `operatingStandards` and `supportedStandards`, but workbook row `282`'s public `_getScanResults()` only serializes `OperatingStandards`. The old isolated rerun `20260410T163026194231` therefore stays blocked for a more precise reason: 5G exact-closes, 6G still emits no same-target external replay block, and 2.4G still drifts to an extra external `be` (`b,g,n,ax` vs `b,g,n,ax,be`), which now looks more like a supported/capability-family readout than a same-source replay of public `OperatingStandards`. So the committed YAML stays unchanged at stale row `284`, details are captured in `plugins/wifi_llapi/reports/D282_block.md`, and the next ready runtime-triage case is `D283`
