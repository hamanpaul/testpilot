...
- `D429 AccessPoint.Neighbour.{i}.ColocatedAP` is now aligned via official rerun `20260415T035709687496`
- workbook authority is row `429`; the old skip placeholder is now replaced by a workbook-faithful AP-only neighbour lifecycle on `WiFi.AccessPoint.{i}.Neighbour.{i}.`, and the rerun exact-closes `empty -> single entry -> empty` on AP1/AP3/AP5 while the created entry reads back `ColocatedAP=0` together with `11:22:33:44:55:66 / 36`、`11:22:33:44:55:77 / 1`、`11:22:33:44:55:88 / 11`, all with `diagnostic_status=Pass`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed aligned case is now `D429 AccessPoint.Neighbour.{i}.ColocatedAP`
- targeted D429/runtime + neighbour/skip-bucket guardrails passed; full repo regression is now `1665 passed`; compare is now `352 / 420 full matches` / `68 mismatches` / `53 metadata drifts`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable single-case closure now moves to `D430 AccessPoint.Neighbour.{i}.Information`
