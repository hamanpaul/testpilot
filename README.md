...
Use `testpilot wifi_llapi` for normal wifi_llapi runs; legacy
`testpilot run wifi_llapi` remains compatible for existing scripts.
...
```bash
TESTPILOT_REPO_URL=file:///home/paul_chen/prj_arc/testpilot bash scripts/install.sh
...
The primary `wifi_llapi` command is:
...
`testpilot run wifi_llapi` remains compatible for existing scripts. Helper
commands remain under the hyphenated `testpilot wifi-llapi ...` group.
...
Commands:
  audit            Audit mode commands.
  brcm-fw-upgrade  brcm_fw_upgrade plugin helper commands.
  list-cases       List test cases for a plugin.
  list-plugins     List available test plugins.
  run              Run tests for a plugin.
  wifi-llapi       wifi_llapi plugin helper commands.
  wifi_llapi       Run wifi_llapi tests.
<!-- END: cli-help marker="testpilot-help" -->
<!-- testpilot-help:end -->

<!-- testpilot-wifi-llapi-help:start -->
<!-- BEGIN: cli-help marker="testpilot-wifi-llapi-help" -->
Usage: testpilot wifi_llapi [OPTIONS]

  Run wifi_llapi tests.

  Usage:   testpilot wifi_llapi [--case CASE_ID] [--dut-fw-ver FW_VER]

Options:
  --case TEXT        Specific wifi_llapi case IDs to run.
  --dut-fw-ver TEXT  DUT firmware version used in report filename.  [default:
                     DUT-FW-VER]
  --help             Show this message and exit.
<!-- END: cli-help marker="testpilot-wifi-llapi-help" -->
<!-- testpilot-wifi-llapi-help:end -->
...
normal testing and intentionally permits only full-run and explicit by-case modes
using `testpilot wifi_llapi`.
...
```bash
testpilot --azure wifi_llapi --dut-fw-ver BGW720-B0-403
```
...
> [!IMPORTANT]
> The first `testpilot wifi_llapi` after pulling new case metadata may
...
`alignment_issues.json` is no longer produced by `testpilot wifi_llapi`; runtime now uses the checked-in template workbook only, and workbook rebuilds go through `testpilot wifi-llapi build-template-report --source-xlsx <path>`.
...
- latest 0506 workbook audit continuation: active RID `74ada64b-2026-05-07T134956Z` now has `D004 kickStation()` applied and validated through audit `verify-edit` / `record` / `decide` / `apply`; focused rerun `20260508T212440407797` reports `Pass / Pass / Pass` and `diagnostic_status=Pass`. `D006 sendBssTransferRequest()` is blocked because workbook row 6 is marked `Pass / Pass / Pass` while its own BTM transcript contains `ERROR: call (null) failed with status 1`, source survey found no source-declared `sendBssTransferRequest()` ODL method, and focused rerun `20260508T220846967896` reproduced 5G/2.4G method errors while only 6G returned `[-1]`. `D009 AssociationTime` is also blocked: source declares the getter and maps it as `AMXC_VAR_ID_TIMESTAMP`, but focused rerun `20260508T223619447864` only returned an ISO timestamp for 6G; 5G had driver association plus positive `sta_info` age while `WiFi.AccessPoint.1.AssociatedDevice.1.AssociationTime?` returned object not found, and AP5 AssociatedDevice data was absent. `D012 AvgSignalStrengthByChain` is blocked as the same AssociatedDevice projection family: source declares a volatile read-only `int32` scalar backed by per-chain RSSI, but focused rerun `20260508T231739051988` only returned 6G `WiFi.AccessPoint.3.AssociatedDevice.1.AvgSignalStrengthByChain=-60`; 5G/2.4G had driver association evidence while `WiFi.AccessPoint.1/5.AssociatedDevice.1.AvgSignalStrengthByChain?` returned object not found. `D013 Capabilities` is blocked: source declares a read-only station capability string, but focused rerun `20260509T000344316109` reached 5G with `AssocMac5g=2c:59:17:00:19:95` and `DriverRrmCapability5g=0x32` while `WiFi.AccessPoint.1.AssociatedDevice.1.Capabilities?` returned object not found, then 6G failed environment preparation with `sta_band_not_ready`. `D014 ChargeableUserId` is blocked: workbook row 14 is non-pass (`To be tested` / `Skip`) and source ties CUI to Enterprise/RADIUS Access-Accept context; focused run `20260509T003124056983` reached AP1 with `assoclist 2C:59:17:00:19:95` but `WiFi.AccessPoint.1.AssociatedDevice.1.ChargeableUserId?` returned object not found. `D015 ConnectionDuration` is blocked: workbook row 15 expects `Pass / Pass / Pass` and source declares a volatile read-only `uint32`, but focused rerun `20260509T005741505140` showed AP1/AP5 AssociatedDevice projection absent despite driver association (`wl0`/`wl2` assoclist), while AP3 exposed an AssociatedDevice instance; the exploratory gate-only YAML edit was reverted through audit `verify-edit` / `apply`. `D016 DownlinkBandwidth` is now blocked with the same AP1/AP5 AssociatedDevice projection failure: source wires `AssociatedDevice[]` reads through `wld_assocDev_getStats_orf` and declares `DownlinkBandwidth` as volatile read-only `uint32`, but focused rerun `20260509T012522496535` reached AP1 only after removing redundant `wpa_cli` gates for exploration; `iw dev wl0 link` and `wl0 assoclist` proved driver association while `WiFi.AccessPoint.1.AssociatedDevice.1.MACAddress?` returned object not found before `DownlinkBandwidth` could be read, AP5 wildcard stayed empty despite `wl2 assoclist`, and AP3 exposed `AssociatedDevice.1`. Current audit buckets are `confirmed=151`, `applied=1`, `pending=146`, `block=117`, `needs_pass3=0`; next strict single-case Pass3 target is `D017`.
...
- TestPilot uses Semantic Versioning with git tags in the form `vX.Y.Z`; the managed baseline for this flow is `v0.2.0`.
- The canonical project version lives in `VERSION`; `pyproject.toml` and `src/testpilot/__init__.py` mirror it for packaging/runtime output and must stay identical.
...
- **Python 3.11+**
- **git**
- **[uv](https://docs.astral.sh/uv/)** — Python 套件管理工具，managed installer 預設使用
- **[serialwrap](https://github.com/paulc-arc/serialwrap)** — DUT / STA UART 通訊用序列埠多工器；managed install 會自動安裝/更新

若開發 checkout 自行管理 serialwrap，可透過環境變數指定路徑：
...
一般操作請使用安裝後的 `testpilot` 指令；開發 checkout 除錯時仍可使用 Python module 入口。

`wifi_llapi` 的主要執行入口如下：
...
`testpilot run wifi_llapi` 仍保留作為既有腳本的相容入口；輔助命令仍維持在
hyphenated `testpilot wifi-llapi ...` group。
...
| 優先序 | 方式 | 啟用方法 |
|--------|------|----------|
| 1 | **Azure OpenAI (BYOK)** | `testpilot --azure wifi_llapi`（互動式詢問） |
...
> [!IMPORTANT]
> 在拉入新的 case metadata 後，第一次執行 `testpilot wifi_llapi`
> 可能會直接改寫 `plugins/wifi_llapi/cases/*.yaml`，把 `D###`、
> `source.row` 與相容的 `id` 片段對齊到 repo 內已提交的 template
> workbook。請檢查這些 diff，並與該次 run 結果一起提交。

`alignment_issues.json` 不再由 `testpilot wifi_llapi` 產生；runtime 現在只使用 repo 內已提交的 template workbook，而 workbook 重建統一透過 `testpilot wifi-llapi build-template-report --source-xlsx <path>`。
...
- TestPilot 採用 Semantic Versioning，git tag 格式固定為 `vX.Y.Z`；此流程的 managed baseline 為 `v0.2.0`。
- canonical project version 放在 `VERSION`；`pyproject.toml` 與 `src/testpilot/__init__.py` 是 packaging/runtime mirror，三者必須保持一致。
