...
- Latest aligned scan-results follow-up:
  ...
  - `D281 getScanResults() Noise` remains blocked, but the blocker is now narrowed to the source-correct same-channel public-spectrum replay
  - official rerun `20260412T011311621294` first exact-closed `Channel / Noise / SpectrumNoise` on all three bands (`36/-100/-100`, `5/-97/-97`, `1/-80/-80`), proving the new direction is source-correct
  - the immediate follow-up rerun `20260412T011644317498` then disproved durability: attempt 1 failed at `step_6g_scan` with `/tmp/_tp_cmd.sh: line 2: syntax error: unexpected end of file`, and attempt 2 still drifted on 2.4G as `Noise=-80` vs `SpectrumNoise=-79`
  - so the local rewrite is rolled back again, committed YAML stays unchanged at stale row `283`, and `plugins/wifi_llapi/reports/D281_block.md` now records the superseding blocker evidence
  - `D282 getScanResults() OperatingStandards` still remains blocked, but the active pWHM ubus path now exposes a concrete sibling oracle candidate
  - current source trace in `pwhm-v7.6.38/src/RadMgt/wld_rad_scan.c` shows `getScanResults()` serializes only `OperatingStandards`, while `WiFi.NeighboringWiFiDiagnostic()` serializes both `OperatingStandards` and `SupportedStandards` from the same `wld_scanResultSSID_t`; live probes already exact-closed 2.4G same-target `04:70:56:D2:22:4F -> b,g,n,ax`, but the diagnostic still reports `FailedRadios = "wl0,wl1"` / `ReportingRadios = "wl2"`, so 5G/6G remain unreplayable and the committed YAML stays unchanged at stale row `284`; see `plugins/wifi_llapi/reports/D282_block.md`
  - `D283 getScanResults() RSSI` is now aligned. The old blocker was the committed full-payload transport shape: isolated rerun `20260411T214050136894` still hung after `setup_env`, but switching the case to a D277-style transport-safe first-object capture removed that failure path while preserving the active 0403 public source family
  - current 0403 source trace still shows `D283 RSSI` and `D286 SignalStrength` are both serialized from the same public `ssid->rssi`, so the committed D283 oracle now locks the durable public facts on the first serialized scan object only: parseable BSSID + numeric `RSSI` + `SignalStrength == RSSI`. Official reruns `20260412T013944779069` and `20260412T014018880783` both exact-closed 5G `38:88:71:2f:f6:a7 / -66 / -66`, 6G `6e:15:db:9e:33:72 / -95 / -95`, and 2.4G `2c:59:17:00:03:f7 / -47 / -47`; the committed YAML is now refreshed from stale row `285` to workbook row `283`, `plugins/wifi_llapi/reports/D283_block.md` is retained as historical resolution notes, `D286` remains blocked on its own unresolved replay model, and the next resume pointer moves to `D284`
  - `D284 getScanResults() SecurityModeEnabled` is now aligned. The old blocker was the unstable 6G target selector, not the public field model itself: the earlier first-WPA-target and associated-BSSID rewrites drifted between `.ROAMTEST_RSNO_P10P_1`, `OpenWrt_1`, and missing same-target LLAPI placeholders, but a D283-style transport-safe first-object capture now locks one durable same-target replay across all three bands
  - official reruns `20260412T015141491861` and `20260412T015235280960` both exact-closed 5G `38:88:71:2f:f6:a7 / WPA2-Personal / WPA2-Personal`, 6G `6e:15:db:9e:33:72 / WPA3-Personal / WPA3-Personal`, and 2.4G `2c:59:17:00:03:f7 / WPA2-Personal / WPA2-Personal`; the committed YAML is now refreshed from stale row `286` to workbook row `284`, `plugins/wifi_llapi/reports/D284_block.md` is retained as historical resolution notes, and the next resume pointer moves to `D285`
  - `D285 getScanResults() SignalNoiseRatio` is now aligned. The old blocker was the raw-driver replay, not the public row itself: 0403 neighboring scan internals still expose only `SignalStrength` plus `Noise`, while the public scan-result model exposes `SignalNoiseRatio` on the serialized object. Switching the case to a D283-style transport-safe first-object capture let the committed replay validate the durable public invariant `SignalNoiseRatio == RSSI - Noise`
  - official reruns `20260412T020817105728` and `20260412T020839239161` both exact-closed 5G `38:88:71:2f:f6:a7 / -66 / -100 / 34`, 6G `6e:15:db:9e:33:72 / -95 / -97 / 2`, and 2.4G `2c:59:17:00:03:f7 / -47 / -80 / 33`; the committed YAML is now refreshed from stale row `287` to workbook row `285`, `plugins/wifi_llapi/reports/D285_block.md` is retained as historical resolution notes, and the next resume pointer moves to `D286`
  - `D286 getScanResults() SignalStrength` is now aligned. The old blocker was the external replay model, not the public row itself: active 0403 source already shows `SignalStrength` and `RSSI` are the same public `ssid->rssi` family, so the committed replay can use the same D283-style transport-safe first-object capture instead of chasing `iw` / raw-driver drift
  - official reruns `20260412T021725610895` and `20260412T021748934770` both exact-closed 5G `38:88:71:2f:f6:a7 / -66 / -66`, 6G `6e:15:db:9e:33:72 / -95 / -95`, and 2.4G `2c:59:17:00:03:f7 / -47 / -47`; the committed YAML is now refreshed from stale row `288` to workbook row `286`, `plugins/wifi_llapi/reports/D286_block.md` is retained as historical resolution notes, and the next resume pointer moves to `D287`
  - `D287 getScanResults() SSID` is now aligned. The old blocker was the stale 6G selector, not the public `SSID` model itself: active 0403 still serializes `SSID` from the parsed scan-result object, and the D283-style transport-safe first-object capture now locks the stable same-target replay on all three bands
  - official reruns `20260412T022708923585` and `20260412T022738004752` both exact-closed 5G `38:88:71:2f:f6:a7 / Verizon_Z4RY7R / Verizon_Z4RY7R`, 6G `6e:15:db:9e:33:72 / **TELUS0227 / **TELUS0227`, and 2.4G `2c:59:17:00:03:f7 / OpenWrt_1 / OpenWrt_1`; the committed YAML is now refreshed from stale row `289` to workbook row `287`, `plugins/wifi_llapi/reports/D287_block.md` is retained as historical resolution notes, and the next resume pointer returns to `D281`
  - superseding `D281 getScanResults() Noise` continuation: the split-step same-channel public-spectrum replay is now transport-clean and source-correct, but still not durable enough to commit
  - official reruns `20260412T023953395097` / `20260412T024026359700` / `20260412T024310730084` settle as `Fail -> pass after retry -> Fail`; 5G and 6G exact-close throughout, while 2.4G alone oscillates between `Noise=-80` and `SpectrumNoise=-78/-82/-80`
  - so the local rewrite is rolled back again, committed YAML stays unchanged at stale row `283`, `plugins/wifi_llapi/reports/D281_block.md` is the superseding blocker authority, and the next resume pointer moves to `D282`
  - superseding `D282 getScanResults() OperatingStandards` continuation: a same-source sibling candidate now exists in `WiFi.NeighboringWiFiDiagnostic()`, which serializes both `OperatingStandards` and `SupportedStandards` from the same `wld_scanResultSSID_t`
  - live probes already exact-closed 2.4G same-target `04:70:56:D2:22:4F -> b,g,n,ax`, but the diagnostic currently returns `FailedRadios = "wl0,wl1"` / `ReportingRadios = "wl2"`, so 5G/6G still lack a durable same-target sibling replay
  - therefore D282 stays blocked, committed YAML remains unchanged at stale row `284`, `plugins/wifi_llapi/reports/D282_block.md` is the superseding blocker authority, and the next resume pointer moves to `D324`
...
- Practical next resume order:
  1. resume the remaining open-set blocker work from `D285`
  2. keep `D324` / `D331` / `D333` blocked unless their drifts are explained with deterministic source-backed corrections
  3. keep `D281` / `D282` / `D285` / `D286` / `D287` as the remaining scan-results blocker slice unless a new deterministic replay closes them
