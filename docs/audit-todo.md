...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=151`, `applied=4`, `pending=140`, `block=120`, `needs_pass3=0`
- latest blocker is `D022 HtCapabilities`: workbook row 22 expects `Pass / Not Supported / Pass` and source declares `AssociatedDevice.HtCapabilities` / `ProbeReqCaps.HtCapabilities` as read-only strings behind `wld_assocDev_getStats_orf`, but focused run `20260509T135805800882` never reached the getters because stale custom 5G WPA3 `sta_env_setup` failed before case-step execution at `iw dev wl0 link` with `Not connected`; because `sta_env_setup` is outside the audit `verify-edit` allowlist, D022 is blocked instead of bypassed
