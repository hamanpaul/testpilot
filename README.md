...
- current lab readiness status: multi-band baseline qualification is complete — `5G/6G/2.4G` all passed `baseline-qualify --repeat-count 5 --soak-minutes 15`; custom-6G hardening made `D019/D027` plain `Pass`, and the old `D032` `sta_band_not_ready` environment failure is gone. The invalid full run `20260412T084218316557` was stopped after early `D007`/`D009`/`D010`/`D011` multi-band instability, then both boards were hard-reset and recovered back to `READY`; patched sequential reruns `20260412T110545613993` (`D009`), `20260412T111048362099` (`D010`), and `20260412T111549474171` (`D011`) all returned `Pass/Pass/Pass` on the same baseline, so the old `D009/D010 FailEnv` and `D011 FailTest` prefix no longer reproduces
...
- latest calibration checkpoint：`D022 HtCapabilities` 已透過 official rerun `20260412T175538121906` 正式對齊。stale alias metadata 已移除，`source.row` 由 `19` 刷新到 workbook row `22`，而真正 durable 的 driver oracle 也已改成解析 `wl sta_info` 的 `HT caps 0x...` bitmask，不再 brittle 地抓 human-readable token；live evidence exact-close `HtCapabilities="40MHz,SGI20,SGI40"` 與 `DriverHt40MHz=1` / `DriverHtSgi20=1` / `DriverHtSgi40=1`。D022 targeted guardrails 為 `1 passed`；疊上 D024/D025/D022 rerun 後，`compare-0401` 已來到 `238 / 420 full matches`、`182 mismatches`、`62 drifts`、actionable workbook-Pass gap `153`。`D020` 維持已驗證的 fail-shaped mismatch；下一個 ready case 是 `D034`
