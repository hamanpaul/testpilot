...
- `D459 getRadioStats().Temperature` is now refreshed via official rerun `20260415T074830179441`
- workbook authority is row `459`; the old row `461` source metadata that still pointed at `WiFi.Radio.{i}.HTCapabilities`, plus the old combined `WiFi.Radio.{i}.getRadioStats()` / `Temperature` shape, are now corrected back to workbook `WiFi.Radio.{i}.` / `getRadioStats()`
- the rerun exact-closes `Pass / Pass / Pass`: 5G/6G/2.4G return valid integer temperatures `87 / 0 / 72`; final status is `diagnostic_status=Pass`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D459 getRadioStats().Temperature`
- targeted D459/runtime tests passed; compare stays `362 / 420 full matches` / `58 mismatches` while metadata drifts drop to `46`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D464 NonSRGOffsetValid`
