**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `361 / 420 full matches`、`59` 筆 mismatch、`47` 筆 metadata drift。** 最新已完成 closure 現在是 `D438 AccessPoint.Security.TransitionDisable`（official rerun `20260415T063258768736`）：workbook authority 已刷新為 row `438`，舊 row `440` generic getter shell 已改寫回 workbook `WiFi.AccessPoint.{i}.Security.` / `TransitionDisable` setter/getter + hostapd `transition_disable` bit replay。current rerun exact-close `Pass / Pass / Pass`：baseline 維持 `TransitionDisable=""` + hostapd `ABSENT`，然後 tri-band getter 與 wl0/wl1/wl2 hostapd 依序 exact-close `WPA3-Personal -> 1`、`SAE-PK -> 2`、`EnhancedOpen -> 8`，最後 restore 回 `"" -> ABSENT`。targeted D438/runtime + budget guardrails 已通過、full repo regression=`1660 passed`。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 survey target 轉到 `D454 getRadioStats().FailedRetransCount`。
