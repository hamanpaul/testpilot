**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `352 / 420 full matches`、`68` 筆 mismatch、`53` 筆 metadata drift。** 最新已提交 closure 現在是 `D429 AccessPoint.Neighbour.{i}.ColocatedAP`（official rerun `20260415T035709687496`）：舊 placeholder skip case 已改寫回 workbook row `429` 與 `WiFi.AccessPoint.{i}.Neighbour.{i}.` / `ColocatedAP` metadata，並沿用已驗證的 AP-only neighbour lifecycle 逐 band exact-close `empty -> single entry -> empty`；current rerun 在 AP1/AP3/AP5 的 add/readback 階段穩定讀回 `ColocatedAP=0`，同時維持 `11:22:33:44:55:66 / 36`、`11:22:33:44:55:77 / 1`、`11:22:33:44:55:88 / 11` 的 BSSID/Channel 對位，`diagnostic_status=Pass`。targeted D429/runtime + neighbour/skip-bucket guardrails 已通過、full repo regression=`1665 passed`。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 closure 轉到同 family 的 `D430 AccessPoint.Neighbour.{i}.Information`。
