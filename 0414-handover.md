**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 仍為 `362 / 420 full matches`、`58` 筆 mismatch、`46` 筆 metadata drift。** 最新已完成 closure 現在是 `D459 getRadioStats().Temperature`（official rerun `20260415T074830179441`）：這筆除了 workbook authority row refresh 之外，也補正了 workbook object/API split。authority 已刷新為 row `459`；舊 row `461` 誤指到 `WiFi.Radio.{i}.HTCapabilities`，而舊 metadata 還保留 combined `WiFi.Radio.{i}.getRadioStats()` / `Temperature` 形狀，現在都已改回 workbook `WiFi.Radio.{i}.` / `getRadioStats()` tri-band Temperature replay。current rerun exact-close `Pass / Pass / Pass`：三個 radio 的 `getRadioStats().Temperature` 分別回 `87 / 0 / 72`，全部維持合法整數讀值並與 workbook pass row 形狀一致。targeted D459/runtime tests 已通過；這筆 closure 讓 strict compare metadata drift 進一步降到 `46`。同一批 focused workbook-faithful survey 也確認 `D454 getRadioStats().FailedRetransCount` 目前仍是 localized blocker：official rerun `20260415T064937785938` 在 5G/2.4G 仍可 exact-close `FailedRetransCount=100/946` 對 `wl0/wl2 counters txfail=100/946`，但 6G 固定漂移成 `FailedRetransCount=0` vs `wl1 counters txfail=740`，因此 exploratory rewrite 已回退、不進 commit。`D371 AccessPoint.AssociatedDevice.DisassociationTime` 仍維持 localized blocker：focused survey runs `20260415T014146461381` / `20260415T015629548681` / `20260415T020725267608` 先後暴露 24G `assoclist` 殘留且 `DisassociationTime` 不變、5G `assoclist` 殘留，以及 6G `step11_6g_post_assoc` serialwrap timeout，因此該 rewrite 仍回退、不進 commit。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog；`D359 AccessPoint.IsolationEnable` 也仍暫停，因 workbook 明確需要 two-station isolation ping，但 current lab/testbed flow 只有標準 single-STA path。systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker；strict compare 仍先列出 `D414/D415` AssociatedDevice RRM cases，但這兩筆 workbook `G` 已明示需要一台 802.11k enable、另一台 disable 的 dual-STA shape，所以暫維持 readiness review；目前下一個 ready actionable 單案 survey target 轉到 `D464 NonSRGOffsetValid`。
