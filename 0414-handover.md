**2026-04-14 superseding note：本檔下半部的大盤分析仍保留 2026-04-13 snapshot；最新 strict compare 已更新為 `333 / 420 full matches`、`87` 筆 mismatch、`58` 筆 metadata drift。** 最新已提交 closure 現在是 `D328 ErrorsSent / SSID stats`（official rerun `20260414T214055064676`）：現有 YAML / runtime metadata 無需修改，official rerun 在 retry `2/2` exact-close tri-band workbook-shaped `Pass / Pass / Pass`，並保留 live direct/getSSIDStats()/driver cross-check evidence `ErrorsSent=0/0/0`。Attempt `1/2` 曾出現 6G transient `ErrorsSent=25` 漂移，但 retry `2/2` 已回到一致 readback；本輪因此只需刷新 compare / handoff，full repo regression 仍沿用上一筆 code-bearing landing 的 `1662 passed`。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，next ready non-blocked compare-open case 改為 `D329 FailedRetransCount / SSID stats`。
