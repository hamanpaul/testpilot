...
- Practical next resume order:
  1. resume the remaining open-set blocker work from `D284`
  2. keep `D324` / `D331` / `D333` blocked unless their drifts are explained with deterministic source-backed corrections
  3. keep `D281` / `D282` / `D286` / `D287` as the remaining scan-results blocker slice unless a new deterministic replay closes them
