...
- compare remains `394 / 420 full matches` / `26 mismatches` / `43` metadata drifts because D527 was already pass-shaped in compare before the workbook-faithful rewrite landed
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D527 SSID WMM AC_VO Stats WmmPacketsSent`
- `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D588 SSID MLDUnit`
