...
- Latest aligned scan-results follow-up:
  ...
  - `D282 getScanResults() OperatingStandards` also remains blocked, but the active public path is now narrowed further. Current 0403 source trace shows the nl80211 parser fills both `operatingStandards` and `supportedStandards`, while public `_getScanResults()` serializes only `OperatingStandards`; the sibling `SupportedStandards` bitmask is not exposed on workbook row `282`
  - the existing isolated rerun `20260410T163026194231` therefore remains blocked for a more precise reason: 5G exact-closes, 6G still has no same-target external replay block, and 2.4G still drifts to an extra external `be` (`b,g,n,ax` vs `b,g,n,ax,be`), which now looks more like a supported/capability-family replay than a same-source replay of the public `OperatingStandards` bitmask. The committed YAML stays unchanged at stale row `284`; see `plugins/wifi_llapi/reports/D282_block.md`
