**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `339 / 420 full matches`、`81` 筆 mismatch、`57` 筆 metadata drift。** 最新已提交 closure 現在是 `D354 Radio.Sensing.Enable`（official rerun `20260415T001339062028`）：stale 的 row `152` / plain `WiFi.Radio.{i}.Enable` getter 已回到 workbook row `354` 的真實目標 `WiFi.Radio.{i}.Sensing.Enable`，每個 band 依序執行 `default -> set 0 -> readback 0 -> restore 1 -> readback 1`，official rerun exact-close tri-band workbook-shaped `Pass / Pass / Pass`，並保留完整 `Sensing.Enable` setter/readback evidence。本輪 targeted D354/runtime + getter-batch guardrails=`195 passed`、full repo regression=`1662 passed`。`D355-D357` 仍是需要 CSI client setup 的 placeholder backlog，暫不視為 ready single-case closure；6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker，因此 next ready non-blocked compare-open case 改為 `D359 AccessPoint.IsolationEnable`。
