...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=157`, `applied=8`, `pending=116`, `block=134`, `needs_pass3=0`
...
- next ready single-case Pass3 target is `D048` (`pass3_pending_source_survey_required`); `D005`, `D007`, `D010`, `D011`, `D023`, `D029`, `D036`, `D038`, `D040`, `D042`, and pass1-confirmed `D027` are in the confirmed bucket
- latest D047 validation: focused `testpilot wifi_llapi --case D047 --dut-fw-ver BGW720-0410-VERIFY` -> `1 passed / 0 failed`, `Pass / N/A / N/A`, compare `full_match_count=0` / `mismatch_case_count=1` against workbook row 47 `Pass / Pass / Not Supported`; blocker is checked-in 5G-only scope versus workbook 6G Pass plus source authority mismatch for standalone AssociatedDevice SupportedHe160MCS
