...
- `D430 AccessPoint.Neighbour.{i}.Information` is now aligned via official rerun `20260415T041252016188`
- workbook authority is row `430`; the old skip placeholder is now replaced by a workbook-faithful AP-only neighbour lifecycle on `WiFi.AccessPoint.{i}.Neighbour.{i}.`, and the rerun exact-closes `empty -> single entry -> empty` on AP1/AP3/AP5 while the created entry reads back `Information=0` together with `11:22:33:44:55:66 / 36`、`11:22:33:44:55:77 / 1`、`11:22:33:44:55:88 / 11`, all with `diagnostic_status=Pass`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed aligned case is now `D430 AccessPoint.Neighbour.{i}.Information`
- targeted D430/runtime + neighbour/skip-bucket guardrails passed; full repo regression is now `1664 passed`; compare is now `353 / 420 full matches` / `67 mismatches` / `52 metadata drifts`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D431 AccessPoint.Neighbour.{i}.NASIdentifier`
