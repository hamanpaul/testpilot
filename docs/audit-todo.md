...
- `D487 Radio Stats WmmFailedBytesReceived AC_BK` is now aligned via official rerun `20260415T103321898419`
- workbook authority is row `487`; the stale row `354` plus `getRadioStats() | grep AC_BK_Stats` replay is retired in favor of direct `WiFi.Radio.{i}.Stats.WmmFailedBytesReceived.AC_BK?` getters plus `wl0/wl1/wl2 wme_counters` `AC_BK` rx failed-byte cross-checks
- the official rerun exact-closes tri-band workbook `Pass / Pass / Pass` at `0 / 0 / 0` with `diagnostic_status=Pass`
- `D486 Radio Stats WmmFailedBytesReceived AC_BE` remains the immediately preceding aligned closure via official rerun `20260415T102002805516`; its earlier driver-side `5G=188 / 6G=0 / 2.4G=0` preprobe did not survive the official rerun, which exact-closed tri-band `0 / 0 / 0`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D487 Radio Stats WmmFailedBytesReceived AC_BK`
- targeted D487/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`). Compare therefore rises to `371 / 420 full matches` / `49 mismatches` while metadata drifts stay at `43`; `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D488 Radio Stats WmmFailedBytesReceived AC_VI`
