**2026-04-15 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 維持 `395 / 420 full matches`、`25` 筆 mismatch、`43` 筆 metadata drift。** 最新已提交 closure 仍是 `D600 WiFi7STARole.NSTRSupport`（official rerun `20260415T173554269251`）：這筆 stale getter-only case 已把舊 source row `416` 退休，改回 workbook row `600`，並把 `results_reference.v4.0.3` 刷回 workbook `Pass / Pass / Pass`。Focused serialwrap survey 與 official rerun 都在 tri-band exact-close `WiFi.Radio.1/2/3.Capabilities.WiFi7STARole.NSTRSupport=1`，單次 attempt 即維持 `diagnostic_status=Pass`。最新 compare overlay refresh 另外補納了兩筆已存在的 authoritative scan rerun：`D277 getScanResults() Bandwidth`（`20260411T205454026707`）與 `D290 getScanResults() CentreChannel`（`20260411T220324862766`）。因此這兩列現在正式從 stale `Fail / Fail / Fail` 修正為 source-backed `Pass / Fail / Pass`，per-band summary 也同步校正成 `5g 397/23`、`6g 395/25`、`2.4g 398/22`。這代表 `D277`、`D289`、`D290` 與 `D020` 現在都屬於 **已驗證 / source-backed fail-shaped mismatch bucket**，而不是新的 workbook-pass-ready closure target。稍早 focused serialwrap survey 仍把 `D588 SSID MLDUnit` 停成 authority/oracle blocker：三個 band 的 public getter 都維持 `MLDUnit=-1`，workbook `H588` 對應的 `wl -i wlX mld_unit 0` / `wl -i wlX mld_unit` 在 current DUT 一律回 `wl: Unsupported`，而 `/tmp/wl*_hapd.conf` 也沒有任何 `mld_unit=` readback 可用；在沒有可重播的 driver/config oracle 前，`D588` 不能升成 workbook closure，repo-visible blocker note 已落在 `plugins/wifi_llapi/reports/D588_block.md`。較早的 focused official rerun `20260415T131457874117` 也已確認 `D508 SSID WMM AC_BE Stats WmmFailedBytesSent` 不能 land 成 workbook `Pass / Pass / Pass`：5G 仍 exact-close `0 / 0 / 0`、6G 仍 exact-close `708116 / 708116 / 708116`，但 2.4G 的 `getSSIDStats()` 與 direct getter 都固定回 `0`，而 `wl2 wme_counters` `AC_BE` tx failed bytes 穩定為 `90`，因此目前仍列 localized **2.4G zero-getter blocker**。較早的 focused serialwrap survey 也仍把 `D524 SSID WMM AC_BE Stats WmmPacketsSent` 停在 tri-band **tx-frame oracle blocker**：`getSSIDStats()` 與 direct getter 在 5G/6G/2.4G exact-close `452 / 452`、`510 / 510`、`547 / 547`，但同窗 `wl wme_counters` `AC_BE` `tx frames` 持續漂到 `896`、`1015`、`1090`；在沒有 stable independent oracle 前，`D524` 不能 land 成 workbook closure，repo-visible blocker note 已落在 `plugins/wifi_llapi/reports/D524_block.md`。既有 localized blockers `D490 Radio Stats WmmFailedBytesSent AC_BE`、`D481 Radio Stats WmmBytesReceived AC_VO`、`D482 Radio Stats WmmBytesSent AC_BE`、`D485 Radio Stats WmmBytesSent AC_VO`、`D454 getRadioStats().FailedRetransCount`、`D371 AccessPoint.AssociatedDevice.DisassociationTime` 之外，`D508`、`D524`、`D588` 目前都維持 blocker；`D355-D357` 仍是 CSI-client placeholder backlog，`D359 AccessPoint.IsolationEnable` 仍暫停在 current single-STA lab shape，strict compare 仍先列出 `D414/D415` AssociatedDevice RRM，但 workbook `G` 已明示需要 dual-STA 802.11k split，所以暫維持 readiness review；systemic active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主，`D211` / `D204` 仍是 parked clarification items，`D182` 仍因 shared 6G baseline path 暫停、`D302` 仍是 source-backed blocker。最新 verification 結果為 targeted runtime/budget guardrails `1251 passed`、full repo regression `1660 passed`；目前 **沒有乾淨的 workbook-pass-ready 單案**，下一個非 blocker revalidation 候選改回 `D020 FrequencyCapabilities`。
