...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=163`, `applied=8`, `pending=104`, `block=140`, `needs_pass3=0`
- latest blocker is `D059 UplinkBandwidth`: workbook row 59 expects `Pass / Pass / Pass`, and source declares `AssociatedDevice.UplinkBandwidth` as a volatile read-only uint32 behind `wld_assocDev_getStats_orf`; focused run `20260509T182916092906` validated the current 5G path with AP1 AssociatedDevice.1 `2C:59:17:00:42:15`, `UplinkBandwidth=20`, and driver `rx nrate ... bw20`, but report shape `Pass / N/A / N/A` mismatches workbook tri-band Pass; adding 6G/2.4G executable coverage is outside the audit allowlist
