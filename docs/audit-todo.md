...
- active audit RID is `74ada64b-2026-05-07T134956Z` against local-only workbook `audit/0506.xlsx`; current buckets are `confirmed=152`, `applied=4`, `pending=137`, `block=122`, `needs_pass3=0`
- latest blocker is `D025 LastDataUplinkRate`: workbook row 25 expects `Pass / Pass / Pass` and source declares `AssociatedDevice.LastDataUplinkRate` as `%volatile %read-only uint32` behind the AssociatedDevice read action `wld_assocDev_getStats_orf`, but focused run `20260509T141740408903` never reached the getter because stale custom 5G WPA3 `sta_env_setup` failed before case-step execution at `iw dev wl0 link` with `Not connected`; because `sta_env_setup` is outside the audit `verify-edit` allowlist, D025 is blocked instead of bypassed
