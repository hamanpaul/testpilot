...
- latest direct-stats continuation after that: `D324 BytesSent` is now aligned as a plain `Pass/Pass/Pass` row. The first real runner replay (`20260411T005329099506`) proved the old workbook `/proc/net/dev_extstats` field `$10` compare was stale, but follow-up source trace plus live serialwrap probes showed the active 0403 vendor path really exposes `BytesSent` through `wl if_counters txbyte`; after rewriting the case to that oracle, real runner rerun `20260411T010328768651` exact-closed on all three bands: 5G `79412272 / 79412272 / 79412272`, 6G `47109866 / 47109866 / 47109866`, and 2.4G `79349200 / 79349200 / 79349200` across direct/getSSIDStats/if_counters. The committed metadata is now refreshed from stale row `248` to workbook row `324`
