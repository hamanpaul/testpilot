...
- `D434 AccessPoint.Neighbour.{i}.R0KHKey` is now aligned via official rerun `20260415T050448284344`
- workbook authority is row `434`; the old skip placeholder is now replaced by a workbook-faithful AP-only neighbour lifecycle on `WiFi.AccessPoint.{i}.Neighbour.{i}.`, and the rerun exact-closes `ABSENT -> "" -> ABSENT` on AP1/AP3/AP5 while the created entry reads back `R0KHKey=""` together with `11:22:33:44:55:66 / 36`、`11:22:33:44:55:77 / 1`、`11:22:33:44:55:88 / 11`; final status is `diagnostic_status=Pass`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed aligned case is now `D434 AccessPoint.Neighbour.{i}.R0KHKey`
- targeted D434/runtime + neighbour/skip-bucket guardrails passed; full repo regression is now `1660 passed`; compare is now `357 / 420 full matches` / `63 mismatches` / `48 metadata drifts`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D435 AccessPoint.Neighbour.{i}.SSID`
