...
- `D020`, `D277`, `D289`, and `D290` now sit together in the verified/source-backed fail-shaped mismatch bucket rather than the workbook-pass-ready queue
- `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and there is currently no clean workbook-pass-ready single case left in the compare-open set; the next non-blocker revalidation candidate falls back to `D020 FrequencyCapabilities`
