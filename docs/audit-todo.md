...
- `D479 Radio Stats WmmBytesReceived AC_BK` is now aligned via official rerun `20260415T091144122493`
- object/api were already aligned to workbook `WiFi.Radio.{i}.Stats.WmmBytesReceived.` / `AC_BK`, but the old source row `346` plus `getRadioStats() | grep AC_BK_Stats` replay is now retired in favor of workbook row `479` with direct `WiFi.Radio.{i}.Stats.WmmBytesReceived.AC_BK?` getters
- the rerun exact-closes workbook `Pass / Pass / Pass` while keeping `diagnostic_status=Pass`: 5G/6G/2.4G direct getter values all match `wl0/wl1/wl2 wme_counters` `AC_BK` RX bytes at `0 / 0 / 0`
...
- historical blocker context for the temporary D257 empty-array failure is retained in `plugins/wifi_llapi/reports/D257_block.md`; latest committed closure is now `D479 Radio Stats WmmBytesReceived AC_BK`
- targeted D479/runtime tests, command-budget guardrail, and full repo regression all passed (`1660 passed`); compare now moves to `366 / 420 full matches` / `54 mismatches` while metadata drifts stay at `43`, `D355-D357` remain in the CSI-client placeholder bucket, `D414/D415` stay in readiness review because workbook `G` requires a dual-STA 802.11k split, and the next ready actionable survey target now moves to `D480 Radio Stats WmmBytesReceived AC_VI`
