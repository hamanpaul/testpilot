**2026-04-14 superseding note：本檔下半部的大盤分析仍保留較早 snapshot；最新 strict compare 已更新為 `337 / 420 full matches`、`83` 筆 mismatch、`58` 筆 metadata drift。** 最新已提交 closure 現在是 `D296 StartACS`（official rerun `20260414T234216396870`）：stale 的 row `221` 與 bare `startACS()` replay 已由 workbook note V 明示的 ACS 前置條件取代——每個 band 先 `AutoChannelEnable=1`、再 `startAutoChannelSelection()`、最後 `startACS()` 並保留 `iw dev wl0/wl1/wl2 info` evidence；official rerun exact-close tri-band workbook-shaped `Pass / Pass / Pass`，同時保留 live evidence `AutoChannelEnable=1`、`startAutoChannelSelection()/startACS() returned` 與 readable `iw dev` radio info。本輪 targeted action-method runtime guardrails + command-budget=`16 passed`、full repo regression=`1662 passed`。6G baseline 目前仍保留 `dut_ocv_not_zero` post-verify drift，因此 active blockers 仍以 `D047` authority conflict + shared 6G baseline manifestations（`D179`、`D181`）為主；`D211` / `D204` 仍是 parked clarification items，next ready non-blocked compare-open case 改為 `D297 StartAutoChannelSelection`。
